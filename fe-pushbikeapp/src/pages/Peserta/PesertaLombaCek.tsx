...
export default function PesertaLombaCek() {
  ...
  try {
    ...
    console.log("DEBUG initialSelected:", initialSelected);
    console.log("DEBUG Peserta API Response:", res.data);
  } catch (err) {
  ...
  try {
    const updateIds = Object.entries(selected)
      .filter(([_, checked]) => checked)
      .map(([id]) => Number(id));

    for (const pesertaId of updateIds) {
      await api.patch(`/lomba/${id}/peserta/${pesertaId}/status`, {
