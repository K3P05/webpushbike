...
export default function LiveHasil() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const pesertaData: PesertaBatch[] = (resPeserta.data ?? []).map((p) => ({
          id_pendaftaran: p.id_pendaftaran,
          nama: p.nama,
          kategori: p.kategori,
          platNumber: p.platNumber,
          community: p.community,
          id_lomba: (p as any).lomba?.id ?? p.id_lomba, // backend bisa kasih nested
          batch: p.batch,
          point1: p.point1 ?? 0,
          point2: p.point2 ?? 0,
        ...
        const batchSize = Math.ceil(
          pesertaData.length / (lombaRes.data.jumlahBatch || 1)
        );

        const emptyBatches: PesertaBatch[][] = Array.from(
          { length: lombaRes.data.jumlahBatch },
          (_, idx) =>
        ...
        pesertaData.forEach((p) => {
          if (p.batch) {
            ...
            const emptySlot = emptyBatches[batchIdx].findIndex(
              (slot) => slot.id_pendaftaran === 0
            );
        ...
        const buatMatch1 = (arr: PesertaSesi[], batchesCount: number) => {
          ...
          for (let i = 0; i < halfBatch; i++) {
            ...
            const batchB = arr.filter(
              (p) => batches1[i + halfBatch] && p.batch === batches1[i + halfBatch]
            );
        ...
        <table className="w-full border-collapse border border-base-light text-textlight">
          ...
          <tbody>
            {batchRanked.map((p, idx) => (
              <tr key={idx} className="hover:bg-base-light/30">
    ...
    matches.map((match, idx) => (
      ...
      >
        ...
        <table className="w-full border-collapse border border-base-light text-textlight">
          ...
          <tbody>
            {match.map((p, i) => (
              <tr key={i} className="hover:bg-base-light/30">
  ...
  if (loading) return <p className="text-textlight">Loading...</p>;
